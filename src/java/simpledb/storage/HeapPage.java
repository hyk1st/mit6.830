 ...
 *
 * HeapPage的每个实例为HeapFiles的一个页面存储数据，并实现了BufferPool所使用的Page接口。
 *
 * @see HeapFile
 * @see BufferPool
 *
...
public class HeapPage implements Page {
    ...
    */
     ...
     *
     *   从页面中删除指定的元组；相应的标题位应被更新，以反映它不再被存储在任何页面上。
     *
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (t == null
                || !t.getRecordId().getPageId().equals((PageId)this.pid)
                || id >= this.numSlots
                || !this.isSlotUsed(t.getRecordId().getTupleNumber())
        ) {
            throw new DbException("this tuple is not on this page, or tuple slot is already empty.");
        }
        this.tuples[id] = null;
        this.header[id / 8] &= (1 << 8) - 1 - (1 << (id % 8));
    }
    ...
    public TransactionId isDirty() {
        // some code goes here
	// Not necessary for lab1
        if (this.dirty) return this.transactionId;
        return null;      
    }
    ...
    private void markSlotUsed(int i, boolean value) {
        // some code goes here
        // not necessary for lab1
        this.header[i / 8] &= (1 << 8) - 1 -(value? 0: (1 << (i % 8)));
    }
